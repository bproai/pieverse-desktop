        ...
        if let Value::Object(map) = value {
            Ok(Prompt {
                ...
                display_order: map
                    .get("display_order")
                    .and_then(|v| v.as_i64())
                    .ok_or("Missing or invalid display_order")?
                    as i32,
                is_active: map
                    .get("is_active")
                    .and_then(|v| v.as_i64())
...
#[tauri::command]
pub async fn sqlite_delete_prompt(id: i64, state: State<'_, SqliteService>) -> Result<(), String> {
    let query = "DELETE FROM prompts WHERE id = ?";
    state
        .execute_parameterized(query, [id.to_string()])
        .map_err(|e| e.to_string())
}
