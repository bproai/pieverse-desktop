...
pub async fn transcribe_audio(audio_base64: String, api_key: String) -> Result<String, String> {
    ...
    let audio_data = match base64::engine::general_purpose::STANDARD.decode(audio_base64) {
        Ok(data) => {
            println!(
                "Successfully decoded base64 data, size: {} bytes",
                data.len()
            );
            data
        }
    ...
    let mime_types = vec!["audio/mp3", "audio/mpeg", "audio/wav", "audio/webm"];
    ...
    Err(format!(
        "All audio format attempts failed. Last error: {}",
        last_error
    ))
}
            ...
            if status.is_success() {
                ...
                match response.json::<Value>().await {
                    Ok(json_response) => {
                        ...
                        if let Some(choices) =
                            json_response.get("choices").and_then(|c| c.as_array())
                        {
                            if !choices.is_empty() {
