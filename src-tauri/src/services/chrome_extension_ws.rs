...
async fn run_chrome_ws_server(
    ...
    println!(
        "Chrome extension WebSocket server started on port {}",
        actual_port
    );
...
async fn handle_chrome_connection(
    ...
    match tokio_tungstenite::accept_async(stream).await {
        Ok(ws_stream) => {
            println!(
                "WebSocket connection established with Chrome extension: {}",
                addr
            );
...
async fn process_chrome_messages(
                    ...
                    if let Ok(text) = msg.to_text() {
                        ...
                        if let Ok(json) = serde_json::from_str::<serde_json::Value>(text) {
                            ...
                            if token.is_none() || token.unwrap() != AUTH_TOKEN {
                                println!(
                                    "Dropping message: Invalid or missing token from {}",
                                    addr
                                );
    ...
    println!(
        "WebSocket connection closed with Chrome extension: {}",
        addr
    );
}
