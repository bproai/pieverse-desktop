...
impl ReferencesService {
    pub fn new(app_handle: AppHandle) -> Self {
        ...
        if !references_file.exists() {
            ...
            let mut file = File::create(&references_file).expect("Failed to create references file");
            file.write_all(json.as_bytes()).expect("Failed to write default references");
        }
